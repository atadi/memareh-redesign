...
$url = $map['SUPABASE_URL'] || $map['NEXT_PUBLIC_SUPABASE_URL']
$secret = $map['SUPABASE_SECRET_KEY']
Write-Host "Using URL: $url"
if (-not $url -or -not $secret) { Write-Error 'Missing URL or secret key in .env.local'; exit 3 }
try {
  $uri = "$url/rest/v1/articles?select=id&limit=1"
  Write-Host "Requesting: $uri"
  $resp = Invoke-RestMethod -Uri $uri -Headers @{ 'apikey' = $secret; 'Authorization' = "Bearer $secret" } -Method Get -ErrorAction Stop
  ...
  # Fallback: try passing the secret key as a query parameter (some clients/Postman use this)
  try {
    $uri2 = "$uri&apikey=$secret"
