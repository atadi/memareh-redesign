...
export function ArticleModeration({ onBack }: { onBack?: () => void }) {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<"list" | "editor" | "comments">(
    "list",
  );
    ...
    <div className="pt-14 px-4">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 mb-4">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <button
            onClick={() => setActiveTab("comments")}
            className="bg-yellow-50 p-4 rounded-lg text-right hover:bg-yellow-100 transition-colors"
          >
            <div className="text-yellow-600 text-2xl font-bold flex items-center gap-2">
              <MessageCircle className="w-6 h-6" />
              {stats.pendingComments}
            </div>
            <div className="text-gray-600">نظرات در انتظار تایید</div>
          </button>
        </div>
      </div>
      ...
      {activeTab === "list" && (
        <div className="bg-white rounded-xl shadow-lg p-4">
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            ...
            >
              <option value="all">
                همه وضعیت‌ها ({statusCounts.all || 0})
              </option>
              <option value="draft">
                پیش‌نویس ({statusCounts.draft || 0})
              </option>
              <option value="published">
                منتشر شده ({statusCounts.published || 0})
              </option>
              <option value="scheduled">
                زمان‌بندی شده ({statusCounts.scheduled || 0})
              </option>
              <option value="archived">
                بایگانی شده ({statusCounts.archived || 0})
              </option>
            </select>
            <select
          ...
          {/* Articles Table */}
          <div className="overflow-y-auto max-h-110 rounded-lg border border-gray-100 scrollbar-thin">
            <style>{`
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b bg-gray-50 sticky top-0 z-10">
                  ...
                  <th className="text-center py-3 px-2">دسته‌بندی</th>
                  <th className="text-center py-3 px-2">نویسنده</th>
                  <th className="text-center py-3 px-2">وضعیت</th>
                ...
                {paginatedArticles.length > 0 ? (
                  paginatedArticles.map((article) => (
                    ...
                    >
                      ...
                      <td className="text-center py-2.5 px-2">
                        <span
                          className={`px-2 py-0.5 rounded text-xs ${STATUS_LABELS[article.status]?.class || "bg-gray-100"}`}
                        >
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-4 pt-4 border-t">
              <span className="text-sm text-gray-600">
                ...
                از {filteredArticles.length} مقاله
              </span>
              <div className="flex gap-2">
                <button
